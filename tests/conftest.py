...
scoped.db. Without this, tests that insert fixed row IDs (e.g. the intake
...
module, and app.db reads SCOPED_DB_URL when its engine is first created.
"""
...
_TEST_DB = Path(tempfile.gettempdir()) / "scoped_test.db"
# Start each session from a clean slate.
if _TEST_DB.exists():
    _TEST_DB.unlink()
os.environ["SCOPED_DB_URL"] = f"sqlite:///{_TEST_DB}"
